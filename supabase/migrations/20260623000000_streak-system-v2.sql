-- Streak system v2 — make the daily-practice ritual compelling.
--
-- Builds on record_daily_practice (v1) by adding:
--   * practice_dates date[]  — a rolling 90-day history so the UI can render a
--     calendar/heatmap "chain" the user doesn't want to break.
--   * milestone XP rewards    — hitting 3/7/14/30/60/100/180/365 grants bonus XP
--     (capped at 1000 so it never trips guard_xp_update's max-jump rule).
-- Server-authoritative and idempotent (safe to paste into Lovable / SQL editor).
...
BEGIN
  ...
  -- Already practiced today → idempotent no-op (return current state).
  ...
  ELSE
    ...
    ELSIF v_gap = 2 AND v_freezes > 0 THEN
      v_freezes := v_freezes - 1;                     -- a freeze bridges one missed day
      v_streak  := v_streak + 1;
      v_froze   := true;
    ELSE
  ...
  -- Milestone + bonus XP (only when newly crossed).
  ...
  -- Append today and keep only the last 90 distinct days (for the calendar).
  v_dates := (
  ...
  -- Log the bonus so XP history stays auditable.
  IF v_reward > 0 THEN
    BEGIN
